...
export const ScanOptions = z
  .strictObject({
    ...
    return_portrait: z.boolean().default(true).meta({
      description: "Whether to return the holder's portrait crop in `images.portrait`.",
    }),
    retain_hours: z
      .number()
      .int()
...
export const ScanTiming = z
  ...
  .meta({
    id: "ScanTiming",
    description:
      "The split of the request's time. Null on a scan made before the split existed " +
      "and read back out of storage: only the engine call was measured then, and it is " +
      "on `duration_ms`.",
  });
